...
document.getElementsByTagName("head")[0].appendChild(s);var u=document.getElementById(r);u.width=320;u.height=480;u.style.top=0+"px";u.style.left=0+"px";
u.tabIndex="1";u.focus();document.body.style.margin="0em";var t=navigator.userAgent;if(o){u.style.width=window.innerWidth+"px";u.style.height=window.innerHeight+"px";
u.addEventListener("touchstart",function(){this.focus();});}else{u.style.width=480+"px";u.style.height=620+"px";u.addEventListener("mousedown",function(){this.focus();
});u.addEventListener("keyup",function(){j(h);},false);}this._css=s;this.field=u;this.ctx=u.getContext("2d");this.fps=30;this.scene=new f();this.scene.parent=this;
},colorToAlpha:function(u,t){var r;for(var s=0,v=n.length;s<v;s++){if(n[s].name==u){r=n[s];r.hex=t;r.index=s;}}r.addEventListener("load",function(){var x=document.createElement("canvas");
var I=x.getContext("2d");var w=this.width;var G=this.height;var F=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;var z=this.hex.replace(F,function(L,N,M,K){return N+N+M+M+K+K;
});var J=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(z);var B=J?{r:parseInt(J[1],16),g:parseInt(J[2],16),b:parseInt(J[3],16)}:null;I.drawImage(this,0,0);
var y=I.getImageData(0,0,w,G);var E=y.data;for(var D=0;D<G;D++){for(var C=0;C<w;C++){var H=D*(w*4)+(C*4);if(E[H]==B.r&&E[H+1]==B.g&&E[H+2]==B.b){E[H+3]=0;
}}}I.putImageData(y,0,0);var A=new Image();A.src=x.toDataURL();n[this.index]=A;});},addChild:function(r){r.ctx=this.ctx;r.field=this.field;r.eventEnable=true;
this.scene.addChild(r);},addChildren:function(){for(var r=0,s=arguments.length;r<s;r++){this.addChild(arguments[r]);}},centerize:function(){var r=this.field.style;
r.marginTop=-parseInt(r.height)/2+"px";r.marginLeft=-parseInt(r.width)/2+"px";r.top="50%";r.left="50%";r.position="absolute";},fitWindow:function(){this.setSize(window.innerWidth,window.innerHeight);
var r=this;window.onresize=function(){r.setSize(window.innerWidth,window.innerHeight);};},setQuality:function(s,r){var t=this.field;t.width=s;t.height=r;
},setSize:function(t,r){var s=this.field.style;s.width=t+"px";s.height=r+"px";},loadingScene:function(r){this.preScene=r;this.preScene.parent=this;},_preLoadEnterFrame:function(){var v=this.field;
this.useEvent();var r=this.ctx;r.clearRect(0,0,v.width,v.height);if(m>0&&this.preScene){this.preScene._enterFrame();}else{if(this.onLoad){this.onLoad();
}var t=this.scene;for(var s=0,x=t.length;s<x;s++){var w=t[s];if(w.onLoad){w.onLoad();}}clearInterval(this.preLoadInterval);var u=this;setInterval(function(){u._enterFrame();
},1000/this.fps);}},_enterFrame:function(){var r=this.field;this.ctx.clearRect(0,0,r.width,r.height);if(this.enterFrame){this.enterFrame();}this.scene._enterFrame();
},pushScene:function(w){var z=this.ctx;var y=this.field;var s=this.scene;for(var v=0,t=s.length;v<t;v++){var x=s[v];x.eventEnable=false;if(x.onSceneremoved){x.onSceneRemoved();
}}for(var v=0,t=w.length;v<t;v++){var u=w[v];u.ctx=z;u.field=y;u.eventEnable=true;if(u.onScenePushed){u.onScenePushed();}}w.parent=this;w.ctx=z;w.field=y;
var r=this.field.style;r.background=null;r.backgroundColor="white";if(w.color){this.setColor(w.color);}if(w.image){this.setImage(w.image);}this.scene=w;
},setColor:function(r){var s=this.field.style;s.background=null;s.backgroundColor=r;},setImage:function(r){var s=this.field.style;s.background="url("+r+") no-repeat center";
s.backgroundSize="cover";},start:function(){var s=this.field;var r=this;this.ctx.clearRect(0,0,s.width,s.height);this.preLoadInterval=setInterval(function(){r._preLoadEnterFrame();
},1000/this.fps);},load:function(){function t(D){var B;if(!D){return B;}var C=D.split(".");var A=C.length;if(A===0){return B;}B=C[A-1];return B;}for(var x=0,u=arguments.length;
x<u;x++){var v=arguments[x];if(v instanceof Array){var w=v[0];var r=v[1];}else{var w=v;var r=v;}var s=t(w);if(s=="wav"||s=="mp3"||s=="ogg"){var v=new Audio(w);
v.name=r;m++;v.addEventListener("canplaythrough",function(){m--;console.log(this.src+" is loaded");});d.push(v);}else{if(s=="TTF"||s=="ttf"){var y=this._css;
var z=document.createTextNode("@font-face{font-family:'"+r+"';src: url('"+w+"') format('truetype');}");if(y.styleSheet){y.styleSheet.cssText=z;}else{y.appendChild(z);
}}else{var v=new Image();v.src=w;v.name=r;m++;v.addEventListener("load",function(){m--;console.log(this.src+" is loaded");});n.push(v);}}}},});var l=function(t,s,w){var r=t.mover;
var v=r[r.length-1];if(v&&v.and){var u=v;}else{var u=new Object();r.push(u);}u.time=0;if(w){u.frame=w;}u.loop=false;u.and=false;u[s]=true;return u;};var k=Atlas.createClass(p,{initialize:function(s,r){this.inherit();
this.x=0;this.y=0;this.rot=0;this._remove=false;this.width=s;this.height=r;this.collisionShape="box";this.mover=[];this.moverIndex=0;this.alpha=1;this.field;
},tween:function(){var r=this.mover;var s=r.length;if(this.moverIndex<s){var t=r[this.moverIndex];if(t.animate){this._animate(t);}if(t.moveTo){this._moveTo(t);
}if(t.moveBy){this._moveBy(t);}if(t.rotateBy){this._rotateBy(t);}if(t.scaleBy){this._scaleBy(t);}if(t.then){this._then(t);}t.time++;if(t.time>t.frame){this.moverIndex++;
if(this.moverIndex==s){if(t.loop){this._refresh();}else{this.stop();}}}}},isQueEmpty:function(){return this.mover.length?false:true;},animate:function(u,r,t){var s=l(this,"animate",t);
s.array=u;s.frameRate=r;s.frameIdx=0;return this;},_animate:function(r){if(r.time==0){this.frame=r.array[0];}if(r.time%r.frameRate==0){r.frameIdx=(r.frameIdx+1)%r.array.length;
