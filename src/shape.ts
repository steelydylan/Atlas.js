...
export class Shape extends Thing {
  ...
  setColorStops(stops: ColorStop[]) {
    ...
    if (this.gradientType == 1) {
      ...
      this.color = this.ctx.createRadialGradient(
        style.x1,
        style.y1,
        style.r1 as number,
        style.x2,
  ...
  _onLoad() {
    ...
    if (this.obj != -1) {
      ...
      var element =
        svgdoc.getElementsByTagName("path")[0] ||
        svgdoc.getElementsByTagName("circle")[0] ||
        svgdoc.getElementsByTagName("rect")[0] ||
        svgdoc.getElementsByTagName("polygon")[0];
      // @ts-ignore TODO
    ...
    const color =
      svg.getElementsByTagName("linearGradient")[0] ||
      svgdoc.getElementsByTagName("radialGradient")[0];
    if (color) {
    ...
    for (let i = 0, n = path.length; i < n; i++) {
      ...
      if (method == "circle") {
        ...
        ctx.quadraticCurveTo(
          tmpx + (p.cpx as number),
          tmpy + (p.cpy as number),
          x,
          y
        );
        ...
        ctx.bezierCurveTo(
          tmpx + (p.cpx1 as number),
          tmpy + (p.cpy1 as number),
          tmpx + (p.cpx2 as number),
          tmpy + (p.cpy2 as number),
          x,
          y
        );
